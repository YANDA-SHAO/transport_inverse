...
def main():
    ...
    output_csv = output_dir / "feature_gaussian_noise.csv"

    # Experiment settings
    num_samples = 5
    seeds = [0, 1, 2]

    # fixed correspondence uncertainty
    offset_std = 0.002

    # sweep observation noise
    noise_list = [0.0, 0.001, 0.005, 0.01, 0.05]
    ...
    # from previous offset experiment
    feature_tau = 1e-2
    lambda_feat = 10.0
    ...
    print("fixed offset_std:", offset_std)
    print()
    ...
    print()
    print("Summary by noise:")
    print("-" * 100)

    for noise_std in noise_list:
        subset = [r for r in rows if r["noise_std"] == noise_std]
